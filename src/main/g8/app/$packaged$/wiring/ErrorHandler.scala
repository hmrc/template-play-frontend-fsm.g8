...
import $package$.connectors.$servicePrefixCamel$ApiError
import $package$.views.html.{InternalErrorView, PageNotFoundErrorView}
import $package$.views.html.components.h1
import $package$.views.html.templates.{ErrorTemplate, GovukLayoutWrapper}
...
class ErrorHandler @Inject() (
...
)(implicit val config: Configuration, ec: ExecutionContext, appConfig: $package$.wiring.AppConfig)
  ...
  override def resolveError(request: RequestHeader, exception: Throwable) = {
    ...
    exception match {
      ...
      case _: $servicePrefixCamel$ApiError => Ok(externalErrorTemplate())
